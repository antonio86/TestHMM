...
void viterbi_log(struct Matrix mat_class, double Test_Vect[], int X[], double Prob[]){
	...
	*/
	...
	memcpy(A_Vect, mat_class.A, N*N);
	memcpy(Mu_Vect, mat_class.MU, D*N*K);
	memcpy(Sigma_Vect, mat_class.SIGMA, D*N*K);
	memcpy(PComp_Vect, mat_class.PComp, N*K);
	
	double LogPdf[M*(N-2)] = { { 0.0 } };

	/*
	// transformation of the Transition matrix in vectors
	for (int n1 = 0; n1 < N ; n1++){
	for (int n2 = 0; n2 < N; n2++){
	A_Vect[n2 + n1*N] = mat_class.A[n1][n2];
	}
	}

	for (int n = 1; n < N-1; n++){
		// transformation of the Emission matrices in vectors
		for(int k = 0; k < K; k++){
			for (int d = 0; d < D; d++){
				Mu_Vect[d + k*D] = mat_class.MU[d][n][k];
				Sigma_Vect[d + k*D] = mat_class.SIGMA[d][n][k];
			}
		PComp_Vect[k] = mat_class.PComp[n][k];
	}
	*/
	...
	for (int n = 1; n < N - 1; n++){
		...
		double * Test_Vect_Array = malloc(D*M*sizeof(double));
		...
		for (int d = 0; d < D; d++){
			...
			for (int m = 0; m < M; m++)
				//Test_Vect_Array[m + d*M] = Test_Vect2[d][m];
				Test_Vect_Array[m + d*M] = Test_Vect[m + d*M];
		}
		for (int k = 0; k < K; k++)
	...
	double * FI = calloc(N*M,sizeof(double));
	int * XX = calloc(N*M, sizeof(int));
	//double FI[N*M] = { { 0.0 } };
	//int XX[N*M] = { { 0.0 } };
	...
	viterbi_end(FI, XX, A_Vect_Array, LogPdf);
	...
	double gg = A_Vect_Array[X[T - 1]-1 + N*(N - 1)];
	for (int t = T - 2; t > -1; t--){
		X[t] = XX[(t+1)*N+X[(t+1)]-1];
		Prob[t] = log(A_Vect_Array[X[t] -1 + N*(X[t+1] - 1)]);
		double gge = A_Vect_Array[X[t] - 1 + N*(X[t+1] - 1)];
		int ggg = X[t] - 1;
		int gg1 = N*(X[t+1] - 1);
	}
